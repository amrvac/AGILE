...
!> |v|, which a rotation about the cylindrical axis leaves invariant, so it
!> stays a single global constant however the velocity's components are
!> written. Written in cylindrical (r, z, phi) components the radial and
!> azimuthal velocity are non-trivial functions of phi (only the axial
!> component stays literally constant), so keeping this state uniform
...
!>
!> This case is also what pins down the sign of the radial momentum at the
...
contains
  ...
  !> cylindrical (r, z, phi) components of the uniform Cartesian *ordinary*
  !> velocity at x. A subroutine rather than an array-valued function, because
  ...
  !> beyond the axis included, whose stored r is negative. That those still
  !> come out right is not a coincidence: the mirror and the per-component
  !> sign the pole copy applies are exactly the change of basis between a
  !> point and its partner half a revolution away, so writing the uniform
  !> Cartesian velocity in the ghost cell's own coordinates reproduces what the
  !> copy has to have put there - lfac0 itself does not change, since it
  !> depends only on |v0|.
